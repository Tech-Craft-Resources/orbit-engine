\chapter{Capítulo 3. Análisis y Diseño del Sistema}\label{capuxedtulo-3.-anuxe1lisis-y-diseuxf1o-del-sistema}

\begin{center}\rule{0.5\linewidth}{0.5pt}\end{center}

\section{Proceso de Levantamiento de Requisitos}\label{proceso-de-levantamiento-de-requisitos}

\subsection{Metodología de Levantamiento}\label{metodologuxeda-de-levantamiento}
...
\tightlist
\item
  \textbf{Entrevistas con usuarios potenciales}: se realizaron sesiones con propietarios y empleados de pymes del sector comercio para identificar sus procesos actuales, puntos de dolor y expectativas sobre una herramienta de gestión.
\item
  \textbf{Análisis de soluciones existentes}: se evaluaron las soluciones descritas en el estado del arte para identificar brechas funcionales.
\item
  \textbf{Elaboración de personas y escenarios}: se construyeron perfiles de usuario (personas) representativos de los distintos roles del sistema, que sirvieron de guía para la priorización de historias de usuario.
\end{enumerate}

\subsection{Perfiles de Usuario}\label{perfiles-de-usuario}
...
\textbf{Perfil 1. Administrador}

\begin{itemize}
\tightlist
\item
  \emph{Rol dentro del negocio}: responsable estratégico y operativo de la organización; típicamente corresponde al propietario o gerente de la pyme. Es quien rinde cuentas sobre los resultados del negocio y toma decisiones de abastecimiento, precios y gestión del personal.
\item
  \emph{Responsabilidades funcionales}: configurar la organización y sus parámetros, administrar el catálogo de productos y categorías, gestionar la base de clientes, supervisar el inventario, registrar y revisar ventas, y dar de alta o de baja a otros usuarios asignándoles roles.
\item
  \emph{Permisos y alcance en el sistema}: acceso completo (lectura, creación, modificación y eliminación) sobre todos los recursos pertenecientes a su organización, incluyendo la gestión de usuarios y la asignación de roles. No tiene acceso a recursos de otras organizaciones, en virtud del aislamiento multi-tenant.
\item
  \emph{Necesidades de información}: indicadores agregados en tiempo real (ventas del día y del mes, productos con stock bajo, top de productos y tendencia de ventas) y reportes exportables que sirvan como insumo para análisis externo o para la rendición de cuentas contable.
\end{itemize}

\textbf{Perfil 2. Vendedor}

\begin{itemize}
\tightlist
\item
  \emph{Rol dentro del negocio}: operador de punto de venta y de atención al cliente; ejecuta la operación cotidiana del negocio en términos de transacciones e interacción con compradores.
\item
  \emph{Responsabilidades funcionales}: registrar ventas multi-producto, consultar la disponibilidad de stock al momento de atender al cliente, asociar la venta a un cliente registrado y consultar el historial de compras del mismo para ofrecer un servicio personalizado.
\item
  \emph{Permisos y alcance en el sistema}: lectura sobre productos, categorías y clientes; creación de ventas y de los movimientos de inventario derivados de éstas; lectura de su propio historial de ventas. No puede modificar el catálogo, eliminar registros ni administrar usuarios.
\item
  \emph{Necesidades de información}: respuesta inmediata en consultas de catálogo y stock, flujo de registro de ventas optimizado para el menor número posible de pasos, y acceso rápido al historial reciente de un cliente durante la atención.
\end{itemize}

\textbf{Perfil 3. Visualizador}

\begin{itemize}
\tightlist
\item
  \emph{Rol dentro del negocio}: analista o asesor (frecuentemente externo, como un contador o consultor) que requiere visibilidad sobre la operación sin intervenir directamente en ella. Su valor agregado reside en la interpretación de los datos del negocio.
\item
  \emph{Responsabilidades funcionales}: revisar indicadores de desempeño, auditar registros de ventas y movimientos de inventario, y exportar listados operativos para su análisis posterior en herramientas externas.
\item
  \emph{Permisos y alcance en el sistema}: acceso de solo lectura sobre todos los recursos de la organización, incluida la capacidad de exportar a Excel los listados de inventario, ventas y clientes. No puede crear, modificar ni eliminar información, lo cual preserva la integridad de los datos cuando el perfil es ejercido por un tercero externo a la empresa.
\item
  \emph{Necesidades de información}: datos consolidados, consistentes y exportables, así como acceso a series históricas que permitan el análisis de tendencias y la elaboración de reportes contables o gerenciales.
\end{itemize}

\subsection{Historias de Usuario Principales}\label{historias-de-usuario-principales}
...
\subsubsection{RF-AUTH. Módulo de Autenticación y Usuarios}\label{rf-auth.-muxf3dulo-de-autenticaciuxf3n-y-usuarios}

\begin{longtable}[]{@{}
  >{\raggedright\arraybackslash}p{(\linewidth - 4\tabcolsep) * \real{0.1330}}
  >{\raggedright\arraybackslash}p{(\linewidth - 4\tabcolsep) * \real{0.7734}}
  >{\raggedright\arraybackslash}p{(\linewidth - 4\tabcolsep) * \real{0.0887}}@{}}
\caption[Módulo de Autenticación y Usuarios]{Módulo de Autenticación y Usuarios}\tabularnewline
\toprule\noalign{}
...
RF-AUTH-03 & El token JWT debe incluir el \texttt{user\_id}, \texttt{organization\_id} y \texttt{role} para que el backend pueda filtrar datos y verificar permisos sin consultas adicionales. & Alta \\
...
\subsubsection{RF-INV. Módulo de Gestión de Inventario}\label{rf-inv.-muxf3dulo-de-gestiuxf3n-de-inventario}

\begin{longtable}[]{@{}
  >{\raggedright\arraybackslash}p{(\linewidth - 4\tabcolsep) * \real{0.1080}}
  >{\raggedright\arraybackslash}p{(\linewidth - 4\tabcolsep) * \real{0.8000}}
  >{\raggedright\arraybackslash}p{(\linewidth - 4\tabcolsep) * \real{0.0880}}@{}}
\caption[Módulo de Gestión de Inventario]{Módulo de Gestión de Inventario}\tabularnewline
\toprule\noalign{}
\begin{minipage}[b]{\linewidth}\raggedright
ID
\end{minipage} & \begin{minipage}[b]{\linewidth}\raggedright
...
\subsubsection{RF-SAL. Módulo de Gestión de Ventas}\label{rf-sal.-muxf3dulo-de-gestiuxf3n-de-ventas}

\begin{longtable}[]{@{}
  >{\raggedright\arraybackslash}p{(\linewidth - 4\tabcolsep) * \real{0.1500}}
  >{\raggedright\arraybackslash}p{(\linewidth - 4\tabcolsep) * \real{0.7444}}
  >{\raggedright\arraybackslash}p{(\linewidth - 4\tabcolsep) * \real{0.1000}}@{}}
\caption[Módulo de Gestión de Ventas]{Módulo de Gestión de Ventas}\tabularnewline
\toprule\noalign{}
\begin{minipage}[b]{\linewidth}\raggedright
ID
\end{minipage} & \begin{minipage}[b]{\linewidth}\raggedright
...
\subsubsection{RF-CUS. Módulo de Gestión de Clientes}\label{rf-cus.-muxf3dulo-de-gestiuxf3n-de-clientes}
...
\begin{longtable}[]{@{}
  ...
  >{\raggedright\arraybackslash}p{(\linewidth - 4\tabcolsep) * \real{0.1000}}@{}}
\caption[Módulo de Gestión de Clientes]{Módulo de Gestión de Clientes}\tabularnewline
\toprule\noalign{}
\begin{minipage}[b]{\linewidth}\raggedright
ID
\end{minipage} & \begin{minipage}[b]{\linewidth}\raggedright
...
\subsubsection{RF-REP. Módulo de Reportes y Dashboard}\label{rf-rep.-muxf3dulo-de-reportes-y-dashboard}

\begin{longtable}[]{@{}
  >{\raggedright\arraybackslash}p{(\linewidth - 4\tabcolsep) * \real{0.1475}}
  >{\raggedright\arraybackslash}p{(\linewidth - 4\tabcolsep) * \real{0.7486}}
  >{\raggedright\arraybackslash}p{(\linewidth - 4\tabcolsep) * \real{0.0984}}@{}}
\caption[Módulo de Reportes y Dashboard]{Módulo de Reportes y Dashboard}\tabularnewline
\toprule\noalign{}
\begin{minipage}[b]{\linewidth}\raggedright
ID
\end{minipage} & \begin{minipage}[b]{\linewidth}\raggedright
...
\begin{longtable}[]{@{}
  >{\raggedright\arraybackslash}p{(\linewidth - 4\tabcolsep) * \real{0.1244}}
  >{\raggedright\arraybackslash}p{(\linewidth - 4\tabcolsep) * \real{0.1429}}
  >{\raggedright\arraybackslash}p{(\linewidth - 4\tabcolsep) * \real{0.7281}}@{}}
\caption[Requisitos No Funcionales]{Requisitos No Funcionales}\tabularnewline
\toprule\noalign{}
\begin{minipage}[b]{\linewidth}\raggedright
ID
\end{minipage} & \begin{minipage}[b]{\linewidth}\raggedright
...
\subsection{Decisiones Arquitectónicas}\label{decisiones-arquitectuxf3nicas}
...
\begin{longtable}[]{@{}
  ...
  >{\raggedright\arraybackslash}p{(\linewidth - 4\tabcolsep) * \real{0.6709}}@{}}
\caption[Decisiones Arquitectónicas]{Decisiones Arquitectónicas}\tabularnewline
\toprule\noalign{}
\begin{minipage}[b]{\linewidth}\raggedright
Decisión
\end{minipage} & \begin{minipage}[b]{\linewidth}\raggedright
...
\subsection{Vista de Componentes de Alto Nivel}\label{vista-de-componentes-de-alto-nivel}

El sistema se organiza en tres componentes principales desplegados de forma independiente:

\begin{figure}
\centering
\pandocbounded{\includegraphics[keepaspectratio,alt={Vista de Componentes de Alto Nivel}]{informe/Vista de Componentes de Alto Nivel.png}}
\caption{Vista de Componentes de Alto Nivel}
\end{figure}

\subsection{Arquitectura del Backend en Capas}\label{arquitectura-del-backend-en-capas}

El backend implementa una arquitectura de tres capas con clara separación de responsabilidades:

\textbf{Capa de API} (\texttt{app/api/}): recibe las solicitudes HTTP, valida los datos de entrada mediante los schemas de Pydantic, verifica la autenticación y autorización, y delega la lógica de negocio a la capa de servicios. Los endpoints siguen la convención REST estándar.

\textbf{Capa de Servicios} (\texttt{app/crud.py}): contiene la lógica de negocio del sistema. Aquí se encuentran las reglas del dominio: descuento de stock al registrar una venta, cálculo de totales, generación del número de factura, agregación de métricas para reportes. Esta capa es independiente del protocolo de transporte (HTTP), lo que facilita el testing unitario.

\textbf{Capa de Acceso a Datos} (SQLModel + SQLAlchemy): gestiona las interacciones con la base de datos mediante el ORM SQLModel. Todas las queries incluyen automáticamente el filtro por \texttt{organization\_id} para garantizar el aislamiento multi-tenant.

\subsection{Estrategia de Multi-Tenancy}\label{estrategia-de-multi-tenancy}

La estrategia de multi-tenancy adoptada es la de \textbf{tabla compartida con discriminador de organización}. Cada tabla de datos de negocio contiene una columna \texttt{organization\_id} (UUID, FK a la tabla \texttt{organizations}) que identifica a qué organización pertenece cada registro.
...
\tightlist
\item
  \textbf{JWT}: al autenticarse, el \texttt{organization\_id} del usuario queda incluido en el token. Cada solicitud al backend extrae este valor del token antes de ejecutar cualquier operación.
\item
  \textbf{Dependencia de base de datos}: la función \texttt{get\_current\_organization()} (inyectada como dependencia en todos los endpoints) verifica que el recurso solicitado pertenezca a la organización del usuario autenticado, levantando un error HTTP 403 si no coincide.
\end{enumerate}
...
\subsection{Flujos Principales del Sistema}\label{flujos-principales-del-sistema}
...
\tightlist
\item
  \textbf{organizations}: representa cada empresa cliente de la plataforma (tenant). Es la raíz de la jerarquía de datos.
\item
  \textbf{users}: usuarios del sistema asociados a una organización y un rol.
\item
  \textbf{categories}: categorías de productos, definidas por organización.
\item
  \textbf{products}: catálogo de productos con control de stock.
\item
  \textbf{customers}: base de datos de clientes de la organización.
\item
  \textbf{sales}: registros de transacciones de venta.
\item
  \textbf{sale\_items}: ítems individuales de cada venta (relación N:M entre sales y products).
\item
  \textbf{inventory\_movements}: historial de todos los cambios en el stock de cada producto.
\item
  \textbf{audit\_logs}: registro inmutable de acciones críticas del sistema.
\end{itemize}

\subsection{Diagrama Entidad-Relación}\label{diagrama-entidad-relaciuxf3n}

\begin{figure}
\centering
\pandocbounded{\includegraphics[keepaspectratio,alt={Diagrama Entidad-Relación}]{informe/OrbitEngine_DER.drawio.png}}
\caption{Diagrama Entidad-Relación}
\end{figure}

\subsection{\texorpdfstring{Tabla: \texttt{organizations}}{Tabla: organizations}}\label{tabla-organizations}

\begin{longtable}[]{@{}
  >{\raggedright\arraybackslash}p{(\linewidth - 4\tabcolsep) * \real{0.1806}}
  >{\raggedright\arraybackslash}p{(\linewidth - 4\tabcolsep) * \real{0.3333}}
  >{\raggedright\arraybackslash}p{(\linewidth - 4\tabcolsep) * \real{0.3889}}@{}}
\caption[Tabla: organizations]{Tabla: organizations}\tabularnewline
\toprule\noalign{}
\begin{minipage}[b]{\linewidth}\raggedright
Columna
\end{minipage} & \begin{minipage}[b]{\linewidth}\raggedright
...
is\_active & BOOLEAN DEFAULT TRUE & Estado de la organización \\
created\_at & TIMESTAMP & Fecha de registro \\
\end{longtable}

\subsection{\texorpdfstring{Tabla: \texttt{users}}{Tabla: users}}\label{tabla-users}
...
\caption[Tabla: users]{Tabla: users}\tabularnewline
\toprule\noalign{}
\begin{minipage}[b]{\linewidth}\raggedright
Columna
\end{minipage} & \begin{minipage}[b]{\linewidth}\raggedright
...
\endlastfoot
id & UUID PK & Identificador único \\
organization\_id & UUID FK & Organización a la que pertenece \\
role & ENUM & admin / seller / viewer \\
email & VARCHAR(255) & Email (único por organización) \\
hashed\_password & VARCHAR(255) & Hash bcrypt \\
full\_name & VARCHAR(200) & Nombre completo \\
is\_active & BOOLEAN & Estado del usuario \\
created\_at / updated\_at & TIMESTAMP & Auditoría \\
\end{longtable}

\subsection{\texorpdfstring{Tabla: \texttt{products}}{Tabla: products}}\label{tabla-products}

\begin{longtable}[]{@{}
  >{\raggedright\arraybackslash}p{(\linewidth - 4\tabcolsep) * \real{0.2500}}
  >{\raggedright\arraybackslash}p{(\linewidth - 4\tabcolsep) * \real{0.2222}}
  >{\raggedright\arraybackslash}p{(\linewidth - 4\tabcolsep) * \real{0.4306}}@{}}
\caption[Tabla: products]{Tabla: products}\tabularnewline
\toprule\noalign{}
\begin{minipage}[b]{\linewidth}\raggedright
Columna
\end{minipage} & \begin{minipage}[b]{\linewidth}\raggedright
...
\endlastfoot
id & UUID PK & Identificador único \\
organization\_id & UUID FK & Multi-tenancy \\
category\_id & UUID FK & Categoría del producto \\
name & VARCHAR(255) & Nombre del producto \\
sku & VARCHAR(100) & SKU (único por organización) \\
cost\_price & DECIMAL(12,2) & Precio de costo \\
sale\_price & DECIMAL(12,2) & Precio de venta \\
stock\_quantity & INTEGER & Stock actual \\
min\_stock & INTEGER & Stock mínimo para alerta \\
max\_stock & INTEGER & Stock máximo sugerido \\
is\_active & BOOLEAN & Estado del producto \\
\end{longtable}

\subsection{\texorpdfstring{Tabla: \texttt{sales}}{Tabla: sales}}\label{tabla-sales}

\begin{longtable}[]{@{}
  >{\raggedright\arraybackslash}p{(\linewidth - 4\tabcolsep) * \real{0.2500}}
  >{\raggedright\arraybackslash}p{(\linewidth - 4\tabcolsep) * \real{0.2222}}
  >{\raggedright\arraybackslash}p{(\linewidth - 4\tabcolsep) * \real{0.4444}}@{}}
\caption[Tabla: sales]{Tabla: sales}\tabularnewline
\toprule\noalign{}
\begin{minipage}[b]{\linewidth}\raggedright
Columna
\end{minipage} & \begin{minipage}[b]{\linewidth}\raggedright
...
id & UUID PK & Identificador único \\
organization\_id & UUID FK & Multi-tenancy \\
invoice\_number & VARCHAR(50) & Número único de factura \\
customer\_id & UUID FK NULL & Cliente asociado (opcional) \\
seller\_id & UUID FK & Usuario que registró la venta \\
total\_amount & DECIMAL(12,2) & Total de la venta \\
discount\_amount & DECIMAL(12,2) & Descuento aplicado \\
payment\_method & ENUM & cash / card / transfer \\
status & ENUM & completed / cancelled \\
created\_at & TIMESTAMP & Fecha de la venta \\
\end{longtable}

\subsection{Estrategia de Índices}\label{estrategia-de-uxedndices}
...
\tightlist
\item
  \texttt{idx\_products\_organization\_id}: acelera la recuperación del catálogo de productos de una organización.
\item
  \texttt{idx\_sales\_organization\_created\_at}: soporta las consultas de ventas por período en el módulo de reportes.
\item
  \texttt{idx\_sale\_items\_product\_id}: acelera la agregación de ventas por producto en los reportes de productos más vendidos.
\item
  \texttt{idx\_users\_organization\_email} (UNIQUE): garantiza unicidad de email dentro de la organización y acelera el login.
\end{itemize}

\subsection{Estrategia de Soft Delete}\label{estrategia-de-soft-delete}

Las entidades principales (usuarios, productos, clientes) implementan soft delete mediante un campo \texttt{deleted\_at\ TIMESTAMP\ NULL}. Un registro se considera activo cuando \texttt{deleted\_at\ IS\ NULL}. Los índices únicos incluyen la cláusula \texttt{WHERE\ deleted\_at\ IS\ NULL} para permitir la reutilización de valores únicos (como email o SKU) tras la eliminación lógica.

\section{Diseño de la Interfaz de Usuario}\label{diseuxf1o-de-la-interfaz-de-usuario}
...
\tightlist
\item
  \textbf{Mínima curva de aprendizaje}: usuarios con conocimientos tecnológicos básicos deben poder completar las tareas principales sin capacitación extensa.
\item
  \textbf{Máximo 3 clics}: cualquier funcionalidad del sistema debe ser accesible desde el punto de entrada actual en no más de tres interacciones.
\item
  \textbf{Feedback inmediato}: cada acción del usuario (guardar, eliminar, registrar una venta) produce una respuesta visual clara (notificaciones toast, estados de carga, confirmaciones).
\item
  \textbf{Consistencia visual}: todos los módulos comparten el mismo sistema de componentes (shadcn/ui), garantizando coherencia en botones, formularios, tablas y modales.
\end{enumerate}

\subsection{Estructura de Navegación}\label{estructura-de-navegaciuxf3n}
...
\tightlist
\item
  \textbf{Dashboard}: vista resumen con KPIs y alertas.
\item
  \textbf{Inventario}: gestión de productos y categorías, historial de movimientos.
\item
  \textbf{Ventas}: registro de nuevas ventas e historial.
\item
  \textbf{Clientes}: base de datos y perfiles de clientes.
\item
  \textbf{Reportes}: exportación a Excel de los listados de inventario, ventas y clientes con filtros aplicados.
\item
  \textbf{Configuración}: gestión de usuarios y perfil de la organización (solo Administrador).
\end{itemize}

\subsection{Sistema de Diseño}\label{sistema-de-diseuxf1o}

La interfaz fue construida sobre \textbf{shadcn/ui}, una colección de componentes accesibles (WAI-ARIA compliant) construidos sobre Radix UI y estilizados con Tailwind CSS. La elección de este sistema de diseño garantiza:
