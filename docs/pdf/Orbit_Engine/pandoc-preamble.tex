...
% Pandoc 3.x define \pandocbounded en su template con lógica de escalado
% (usa \Gscale@div de graphicx para ajustar al ancho/alto del texto).
% NO redefinir como no-op aquí; dejar que la definición nativa de Pandoc
% prevalezca. Si el template de Pandoc no está presente (compilación directa
% de .tex sin Pandoc), se provee un fallback seguro.
\makeatletter
\@ifundefined{pandocbounded}{% fallback si no viene del template Pandoc
